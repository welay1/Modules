...
packages_needed <- c("tidyverse","learnr", "MASS", "lessR", "gradethis")
...
Welcome to Module 6: **Impact of Multicollinearity**. In this module, you will learn: 
  ...
  * To use R to visualize confidence intervals of coefficients from both simple and multiple linear models. 

  * To explain how increasingly correlated predictors change the widths of confidence intervals for both simple and multiple linear models. 

  * To explain how unmeasured confounders might impact estimated coefficients in a linear model.  
...
First, we will review correlation and the correlation coefficient, $r$. Recall that the correlation coefficient measures the strength of linear relationship between two variables, $X$ and $Y$, and is calculated as $r=\frac{\Sigma[(x_i-\bar{x})(y_i-\bar{y})]}{\sqrt{\Sigma(x_i-\bar{x})^2*\Sigma(y_i-\bar{y})^2}}$.The value of $r$ falls in the range of $[-1,1]$ with values near $-1$ denoting a strong negative relationship, and values near $1$ denoting a strong positive relationship. Values near 0 denote weaker relationships. 

TODO: Message for the first one

```{r q0, include=TRUE}
question("Which statement is true regarding correlation between two measurements?",
         answer("The correlation coefficient tells us whether these measurements are linearly related."),
         answer("If the measurements are strongly linearly related, then a linear regression model can be used to represent this association.",correct = TRUE),
         answer("The correlation coefficient can be used to describe the association between 3 or more measurements."),
...
question("Which of the below statements is correct regarding this true relationship?",
         answer("$X_3$ has no linear association to $Y$."),
         answer("In the presence of the other predictors, a one-unit increase in $X_2$ yields a 3-unit increase in $Y$."),
         answer("On average, when other predictors are held fixed, a one-unit increase in $X_1$ increases $Y$ by 2 units.", correct = TRUE),
...
```{r confint1_setup, include=FALSE}
# fill in the values from above:
n = 50
errorvar = 2.25
betas <- c(1,2,3,0)
...
question("Which statement is true regarding the three simple linear models?",
         answer("The estimated slope in each model is not equal to the corresponding slope in the true relationship", correct = TRUE),
         answer("In only one of the true relationship and the simple linear model using $X_3$, $X_3$ has no linear association with $Y$."),
         answer("The estimate of the error variance matches the true error variance in the population."),
...
```{r linearity1_setup, include=FALSE}
# fill in the values from above:
n = 50
errorvar = 2.25
betas <- c(1,2,3,0)
...
```{r linearity2_setup, include=FALSE}
# fill in the values from above:
n = 50
errorvar = 2.25
betas <- c(1,2,3,0)
...
question("Why are the confidence intervals on the coefficient of each predictor narrower in the multiple linear model compared to the separate simple linear models?",
         answer("The predictors we generated are strongly correlated."),
         answer("The error variance is not estimated correctly in the simple linear models."),
         answer("The true relationship is not estimated correctly in the simple linear models."),
...
question("Which model(s) seem to be estimating the true association of each predictor with the response?",
         ...
         answer("Simple model with $X_1$."),
         answer("Simple model with $X_2$. "),
         answer("Simple model with $X_3$. ")
)
```
...
question("Which of the below is true regarding these two simple linear models?",
         ...
         answer("More than one of these is true.", correct = TRUE)
)
```
