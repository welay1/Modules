...
output$modelPlot <- renderPlot({
  ...
  ggplot(data, aes(x = x, y = y)) +
    geom_point() +
    geom_smooth(method = "lm", formula = y ~ poly(x, ifelse(input$modelType == "Linear", 1, ifelse(input$modelType == "Quadratic", 2, 3))), se = FALSE, colour = "red") +
    ...
    output$modelTypeUI <- renderUI({
        if (input$plotType == "Linearity") {
            selectInput("modelType", "Model Type:",
                        choices = c("Linear" = "Linear", "Quadratic" = "Quadratic", "Cubic" = "Cubic"), selected = "Linear")}
    })
...
fittedModel <- reactive({
    set.seed(301)
    x <- runif(100, 0, 10)
    data <- data.frame(x = x, y = 1 + 5 * x^3 + rnorm(100, 0, 200))
...
output$linearPlot <- renderPlot({
    ...
    ggplot(data, aes(x = x, y = y)) +
        ...
        geom_smooth(
            method = "lm",
            formula = y ~ poly(x, ifelse(input$modelType == "Quadratic", 2, ifelse(input$modelType == "Cubic", 3, 1))),
            se = FALSE,
            colour = "red"
        ) +
        labs(title = paste("Model Type:", input$modelType), x = "X", y = "Y") +
        ylim(-800, 5000) +
        theme_minimal()
})
