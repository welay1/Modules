...
* To describe how a simulation is used to understand statistical properties of estimators. 
...
* To adapt existing simulation code in R to study violated Homoscedasticity 
...
* To adapt existing simulation code in R to study violated Homoscedasticity and Violated Linearity/Model misspecification. 
...
* To adapt existing simulation code in R to study violated Homoscedasticity, Violated Linearity/Model misspecification, and Uncorrelated Errors.  
...
* To adapt existing simulation code in R to study violated Homoscedasticity, Violated Linearity/Model misspecification, Uncorrelated Errors, and Normality. 
...
```{r, context = "server"}
#### For Independence Among Predictors
...
# Conditional UI for the variance multiplier
    output$varianceMultiplierUI <- renderUI({
        if (input$plotType == "Homoscedasticity") {
            sliderInput("varianceMultiplier", "Variance Multiplier:",
                        min = 0, max = 10, value = 0, step =2)
        }
    })

output$homoscedasticityPlot <- renderPlot({
  print(paste("Variance Multiplier:", input$varianceMultiplier))

  set.seed(302) # Ensure reproducibility

  # Simulated data
  x <- rnorm(500)
  # Generate non-constant variance for y
  variance <- 1 + input$varianceMultiplier * abs(x)
  # Generate y values based on the specified variance
  e <- rnorm(500, sd = sqrt(variance))
  y <- 1 + 4 * x + e
  ...
  # Plot y vs. x and include the line of best fit
  ggplot(data.frame(x, y), aes(x, y)) +
    geom_point() +
    geom_smooth(method = "lm", se = FALSE, color = "red", linetype = "dashed") +
    labs(x = "Predictor (x)", y = "Response (y)",
         title = "Y-X Plot Showing Heteroscedasticity") +
    theme_minimal()
})

#### Plotting for Linearity

# Conditional UI for the model type
output$modelTypeUI <- renderUI({
    if (input$plotType == "Linearity") {
        selectInput("modelType", "Model Type:",
                    choices = c("Linear", "Quadratic", "Cubic"), selected = "Linear")
    }
})

set.seed(301)
x <- runif(100, 0, 10)
y <- 1 + 5 * x^3 + rnorm(100, 0, 200)  # Generating data with a cubic trend
data <- data.frame(
  x = x,
  y = y)

    
# Reactive expression for fitted model and prediction function
fittedModelAndPredict <- reactive({
  if (input$modelType == "Linear") {
    lm(y ~ x, data = data)
  } else if (input$modelType == "Quadratic") {
    lm(y ~ poly(x, 2), data = data)
...
# Model statistics for Linearity
output$modelStats <- renderTable({
    if (input$plotType == "Linearity") {
        model <- fittedModelAndPredict()$model
        broom::glance(model)[, c("r.squared")]
    }
})
    
output$plotUI <- renderUI({
    if (input$plotType == "Independence") {
        plotOutput("independencePlot")
    } else if (input$plotType == "Homoscedasticity") {
        plotOutput("homoscedasticityPlot")
    } else if (input$plotType == "Linearity") {
...
fluidPage(
    titlePanel("Assumption Violations"),

    sidebarLayout(
        sidebarPanel(
            selectInput("plotType", "Select Plot Type:",
                        choices = c("Linearity", "Homoscedasticity", "Independence")),

            uiOutput("varianceMultiplierUI"),
