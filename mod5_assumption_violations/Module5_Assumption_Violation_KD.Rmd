...
As we learned, to violate homoscedasticity, the variance of the error term must be constant and consequency independent of the predictors i.e. be uncorrelated. To determine if the assumption holds, we tend to use diagnostic plots such as Residual Plots where Residual values ( ${r_i = y_i - \hat{y}_i}$ ) are plotted against the Fitted values (X). 
...
In the plot below, the slider allows you to adjust the variance for the error term. For a variance multiplier greater than zero, the variance of the error term becomes dependent on the fitted values, as the error terms become correlated. 
