...
In this module, we will investigate the consequences of violating the assumptions of linear regression. Specifically, we will look at how confidence intervals of coefficients are impacted by violated assumptions. 

We will run a simulation to mimic how drawing different samples from the population produces different coefficient estimates and confidence intervals. 

Note: There are hints for some questions, so make sure to check them if you get really stuck.
...
Usually, we do not know the true population relationship -- the whole purpose of linear regression is to *estimate* this relationship. However, for the purpose of this exercise, we will assume we do know this information, as we will be replicating drawing samples from some population. As such, we first establish the "ground truth" -- the true population relationship between the predictors and response. Then we take many samples from this population to obtain an estimate of the sampling distribution of each coefficient. Finally, we compute our confidence interval for each coefficient in each sample we take and see whether we obtain the right proportion of intervals that contain the true value of that parameter (i.e. the coverage).
...
Fill in the gaps below to run 2000 iterations of the simulation and save the resulting confidence interval of each coefficient for each run.
...
Remember that coverage here refers to the **proportion of confidence intervals based on samples from this population that contain the true value of the coefficient.** So by taking the 2000 samples from this population and computing confidence intervals each time, we can check if our 95\% confidence intervals actually capture the true value of the coefficient 95\% of the time. So for a confidence interval $[a, b]$, we would find the coverage by $$\sum_{i=1}^{2000}\frac{1_{\{\beta \in [a, b]\}}}{2000}.$$ 

This amounts to checking if the true value of the coefficient is inside the interval for each run (a yes or no answer) and then adding up how many yes' we found. This can be broken up into 4 pieces in our code:
...
In this part, we will examine the homoscedasticity (constant variance) assumption and how the violation of it could impact the confidence intervals we would build for the estimated coefficients.

Let's start by reviewing the definition for the homoscedasticity (equal variance) assumption.  
...
Now let's break the homoscedasticity assumption in our simulation. We will use the same simulation template as the previous section, but instead of having assumptions hold, we will break constant variance. 
