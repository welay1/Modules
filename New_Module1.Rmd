...
A year ago, your best friend launched a cleaning company, but it's not yet profitable. She believes that some employees might not be performing effectively, resulting in no additional rooms being cleaned and, consequently, no profit. She seeks your help to confirm her suspicions.


You request records from the past year detailing the number of workers sent (CrewSize, X) and rooms cleaned (Rooms, Y) **to test if more workers lead to more cleaned rooms.** 

Unfortunately, *she only has data for one month*: **February**, rather than the entire year. 
...
question("Which of the following is true about analyzing the business based on just one month's data instead of a full year's data?",
         ...
         answer("If we examined data from another month, the results would be consistent with this month's findings", message = "Every month's data (sample) from the year will show variations due to sampling. Thus, it might not align perfectly with the results from the month our friend provided."), 
...
To begin, let's produce a 5-number summary of each variable in the data. The name of the dataset is "data" and it is loaded for you to use. You can type your code into the box below, and if you are stuck, you can click the hint button to help you with the R code and syntax.
...
We now want to visualize the sample data to determine the association between our two variables. Considering the numerous plot options, which is the most suitable for our data? . 
...
Generate a scatterplot using "CrewSize" on the x-axis and "Rooms" on the y-axis, ensuring to label the axes and title appropriately. Fill in the code gaps with the relevant dataset variables to complete the plot.
...
In Section 1, we observed a linear relationship between the number of employees and the rooms cleaned. This suggests a potential **line of best fit** that can elucidate this association. 

By determining the equation of this line, we can gain insights into how the increment in employees affects the number of rooms cleaned.

Recall Figure 1 of Part 1.2, where the scatterplot displays discrete crew sizes as distinct vertical clusters. Each cluster represents the **conditional distribution** of rooms cleaned ($Y$) for a specific crew size ($X$). We can examine the average Y within each distribution.

Remember, a *conditional distribution* provides insight into the relationship between two random variables. It does this by analyzing the distribution of one variable, $Y$, for each distinct value of the other variable, $X$. Essentially, for every unique value of $X$, we examine the behavior or distribution of $Y$.
...
We just observed that your friend sent crews of sizes 1 through 5 to jobs in February. We'll calculate the average rooms cleaned for each crew size, denoted as $E(Y \mid X = x)$, representing the conditional mean of rooms cleaned for a specific crew size.
...
In Figure 1 of Section 1.2, we plotted CrewSize (x-axis) against Rooms (y-axis). Now, in this section, we'll integrate the five conditional means that we just obtained.

Also, let's color them in green to distinguish them!!
...
question("What do you notice about the relationship between the means as X (CrewSize) increases",
         answer("We see a reasonably strong positive linear association.", correct = TRUE, message = "The association is linear and positive, characterized by its upward trend, absence of curvature, and the close clustering of points, indicating its strong characteristic."),
         answer("We see no linear association present.", message = "The scatterplot clearly shows an upward trend, with the number of rooms increasing as the crew size increases. Additionally, the blue points, representing the conditional means, further emphasize this positive linear trend across different crew sizes. Thus, there is a visible linear association between the two variables."), 
         answer("We see a weak positive linear association.", message = "The scatterplot indicates a clear upward trend with the number of rooms consistently increasing as the crew size increases. The blue points, representing the conditional means, solidify this trend. The data points are relatively closely clustered around these means, suggesting a strong positive linear relationship between the two variables, not a weak one."),
         answer("We see a reasonably strong nonlinear association.", message = "The scatterplot, especially with the blue points representing the conditional means, clearly shows an upward linear trend as the crew size increases. The data suggests a strong positive linear relationship between the two variables, rather than a nonlinear association."), allow_retry = TRUE
)
```
...
You might have noticed that the second multiple choice answer above allows us to touch on why there is only one line of best fit. In the next section, we'll dive into why that is the case!

## Section 4: Constructing the Line of Best Fit

In the previous section, we defined the line of best fit as: $$y = -1.04 + 3.96x.$$  This equation represents a distinct linear relationship derived from the data. The coefficients (-1.04 and 3.96) were determined by finding a line that minimizes the cumulative squared deviations between each observed data point and its corresponding point on the line.

*Let's unpack that a little*: For each data point, we can compute a 'residual', which is the difference between the observed value (y) and the value predicted by our line ($3.96x - 1.04$). The squaring part $(y - (mx + b))^2$ ensures that we account for these residuals in a consistent way, whether they are positive or negative, making our optimization problem smoother.

Each of our 100 data points has a difference from our line, like a small gap. We square these gaps and then add them all up. This total is what we call the **residual sum of squares (RSS)**. The RSS is a number that measures the total discrepancy between our data and our line of best fit. It's like a scorecard; *the smaller the RSS, the better our line fits the data*. We aim for the line that gives us the smallest RSS because that line fits our data the best. Minimizing this RSS yields a *unique* line that closely follows the data points and connects the estimated average rooms cleaned for each crew size.
...
In the applet below, we've dispersed our familiar data for clarity. Adjust the slope and intercept as you like. The applet will display the corresponding RSS and distance lines. 
...
Congratulations! You have successfully completed this learning module! Using just the data from February, you can now confidently inform your friend about the average increase in the number of rooms cleaned for each additional member added to a cleaning crew over the past year.
