...
We have seen that a simple linear model estimates the population relationship between a response (Y) and a predictor (X) using a sample. The population relationship is $$Y = \beta_0 + \beta_1 X + \epsilon$$, and we use the process of least squares to estimate a slope $\beta_1$ and an intercept $\beta_0$. 

We know that more complex relationships with multiple predictors can be modeled in the form $$Y = \beta_0 + \beta_1 X_1 + \beta_2 X_2 + \ldots + \beta_p X_p + \epsilon.$$ While it is common that predictors are *quantitative*, *qualitative* properties that inform the population relationship. 
...
First, we will learn how to include qualitative predictors in a simple linear model. 

We will work with a LEGO dataset (called `lego`). This dataset contains information on 75 LEGO brick sets sold on Amazon. The response variable of interest will be `Amazon_Price`, the price that the set was sold on Amazon. We have a number of different possible predictors in our data:
  ...
  - `Price`: The original sale price of the set, which may be different from `Amazon_Price`
...
In the code chunk below, identify the levels of the variable `Theme`, and the number of observations in each level. You can do this using the `table()` function, with the syntax `table(data$variable)`.

```{r summaryTheme, exercise=TRUE, exercise.lines=6}
table(lego$Theme)

```

```{r summaryTheme-hint-1}
# you can use a built-in function 
table(data$variable)
```

The first row of the table are the levels, and the second row is the number of observations that take each level. 

Do the same thing, but with the variable `Pieces`. 

```{r summaryPieces, exercise=TRUE, exercise.lines=6}
table(lego$Pieces)
```

Finally, find the levels of `rating`.

```{r summaryRating, exercise=TRUE, exercise.lines=6}
table(lego$rating)
```
...
While useful, qualitative variables can be difficult to work with. Sometimes we must manipulate qualitative variables to make them more useful for modelling. 

### Named Categories

Qualitative predictors may often have levels that are names or words (called a *string* or *character* data type). For example, `Theme` has levels that are words. It is reasonable that a Lego set's theme may impact its price -- but how do we include character data types in a linear model? 

Sometimes, it is useful to manipulate qualitative data to make it easier to use. We will cover a few methods of data manipulation, with the first being *subsetting*. To subset is to select only certain levels of the variable. To do this, we can use true/false conditions (called *booleans*) to check which rows contain certain values. We use `variable == "value"` to check this, with two equals signs. We can also check whether a certain row does not hold a specific value by using `variable != "value"`. 
...
```{r subset, exercise=TRUE, exercise.lines=6, exercise.blanks = "___+"}
...
We could also achieve the same result of collecting only rows that are not Friends by using an OR argument, which looks like a vertical bar: 
...
Notice that the coefficients are labelled with values of the qualitative predictor *Theme*, but only two out of the three levels are present. This is because the intercept is actually equal to the average value of Amazon_Price among the City sets: E(Y | X = "City") =  `r mean(lego$Amazon_Price[lego$Theme=="City"])`. The slope for Friends is the **difference** between the mean Amazon Price of City sets and Friends sets: E(Y | X = "Friends") - E(Y | X = "City") =  `r mean(lego$Amazon_Price[lego$Theme=="Friends"]) - mean(lego$Amazon_Price[lego$Theme=="City"])`. The same is true of the slope for DUPLO\\xae.  
...
However -- does it make sense to treat the predictor as continuous? The rating scale only goes from 1 to 5, and it cannot take values that are not integers. Further, it is inherently a qualitative measurement (someone judged the set on "goodness" and gave it a rating). It also may not make sense to assume that, if we compare sets with a rating of 1 and 2, it would necessarily have the same effect on the mean Amazon Price as sets with ratings of 4 and 5 (i.e. the change may differ for different ratings). 

Therefore, it's important to always think about what variables are actually representing. We can tell R that we want it to use rating as a qualitative variable by using the `as.factor(variable)` command, which forces R to see this variable as qualitative rather than numerical. Try this below to see how it changes the model estimates.
...
Notice again that only 4 out of the 5 levels are present. This is because the intercept is equal to the slope for the average Amazon Price among sets rated '1'. Similar to before, we would interpret the coefficients as the change in the average Amazon Price of a set with rating of X (2, 3, 4, or 5) compared to a rating of 1.

```{r q4, echo=F}
question("Sets with which rating have a lower Amazon Price compared to sets with a rating of 1?",
         ...
         answer("Only sets with a rating of 2 and 5.", correct=T, message="Since the intercept is the average Amazon Price for a set with rating of 1, coefficients that are negative tell us that sets with those ratings have lower average Amazon Prices compared to this reference."),
         ...
         answer("Only sets with a rating of 3 and 4.", message="Ratings with positive coefficients actually tell us that sets with these ratings have a higher Amazon Price than a set with a rating of 1."),
         allow_retry = T)
```
...
From the previous portion, we have noticed that when we fit separate simple linear models for each of the different sets, we find a common slope but a different intercept. To better see this with our data, we can change our scatterplot to include these estimated trends.
...
You should notice that your intercept in the multiple linear model is estimated to be about 10 with an estimated slope for Friends of about -7. To interpret the coefficients, notice that the intercept is the mean Amazon Price for a *City* set with 0 pieces. This value is about 10, which is very close to the 9.4 we saw for an intercept in the simple linear model for City. So the intercept in the multiple linear model nearly matches the one from the City simple model.
