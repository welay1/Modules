...
* How to include and interpret qualitative variables in linear models

* How to include and interpret interaction terms in linear model 

Remember that if you get stuck on a coding exercise, you can click the **Hints** button for help. 

## Motivation

We have already seen that a simple linear model estimates the population relationship between a predictor and a response using a sample. The population relationship is $Y = \beta_0 + \beta_1 X + \epsilon$, and we use the process of <mark>least squares</mark> to estimate $\hat{y} = \hat{\beta_0} + \hat{\beta_1}X$, where $\hat{\beta_0}$ is the intercept and $\hat{\beta_1}$ is the slope. 


We know that we can extend simple linear regression to model more complex relationships of the form $Y = \beta_0 + \beta_1 X_1 + \beta_2 X_2 + \ldots + \beta_p X_p + \epsilon$. While we have only seen quantitative (numerical) predictors so far, we can also include qualitative (categorical) predictors in linear regression models. 
...
Before we jump to creating a mulitple linear model, let's focus on working on how to include qualitative information in a simple linear model.
...
To know whether a variable is qualitative, we have to check the levels of the variable. <mark>Levels</mark> are all possible values a variable can take. 
...
Qualitative predictors may often have levels that are names or words (called a *string* or *character* data type). For example, `Theme` has levels that are words. We may want to include Theme in a model predicting price, but how do we include character data types in a linear model? 
