...
We have already seen that a simple linear model estimates the population relationship between a response (Y) and a predictor (X) using a sample. The relationship being estimated is $$Y = \beta_0 + \beta_1 X + \epsilon$$ where we use the process of least squares to estimate a slope $\beta_1$ and an intercept $\beta_0$. 

We have also now seen that we can use multiple predictors in a model to estimate a more complex relationship of the form $$Y = \beta_0 + \beta_1 X_1 + \beta_2 X_2 + \ldots + \beta_p X_p + \epsilon.$$ where the $p$ represents the number of predictors used in the model. While it is common that our variables are continuous in nature, there are many characteristics/properties that are qualitative in nature that are useful in understanding the population relationship. When working with qualitative predictors, we need to treat them a little bit differently than continuous ones. 

In this module, we will explore how we can use qualitative predictors in a multiple linear model to estimate population relationships, and how this changes our interpretation of the coefficients in the model.

## Section 0: Recognizing Categorical Variables

Before we jump into understanding how we use qualitative predictors alongside other predictors, let's focus on working on how to include this type of information in a simple linear model.
...
To do this, we will work with a LEGO dataset (called `lego`). This dataset contains information on 75 LEGO brick sets sold on Amazon. The response variable of interest will be `Amazon_Price`, the price that the set was sold on Amazon. We have a number of different possible predictors in our data:
  ...
  - Price: The original sale price of the set
  - Year: The year that the set was sold on Amazon
  - Ages: The ages that each set is listed as being appropriate for kids
  - Pages: The number of pages of the instruction booklet
  - Minifigures: The number of figurines included in the set
  - Packaging: The type of packaging of the set
  - Weight: The weight of each set
  - Unique_Pieces: The number of unique piece types in the set
  - Availability: The type of availability of the set (exclusive, promotional, retail)
  - Size: The size classification of the set (Large or Small) based on the number of pieces
  - rating: The rating (on a scale of 1 to 5) of each set.

In the below code chunk, produce a table of counts for the variables `Theme`, `rating`, and `Pieces`.

```{r summary, exercise=TRUE, exercise.lines=6}
# add your code below
...
```{r summary-hint-3}
# if using tidyverse, you would use
dataset %>% count(variable)
```


Based on your summaries of these variables, we need to decide whether we would treat each one as a numerical predictor or as a qualitative predictor.
...
Qualitative variables are really useful as predictors, but can be difficult to work with. Before seeing how we can use them in a model, we will look at some skills for manipulating the different types of qualitative variables.

### Named Categories

Qualitative predictors are most recognizable when the values they take are names or words, such as the variable `Theme` in our LEGO dataset. They can sometimes be tricky to work with, so it's important to know how they differ from quantitative variables. 

The first important step in working with qualitative variables is to know how to subset or select only certain levels/values of the variable. To do this, we can use true/false conditions (called booleans) to check is a certain row holds a specific value or not. We use `variable == "value"` to check this. We can also check whether a certain row does not hold a specific value by using `variable != "value"`. 

Below, fill in the two lines of code that 1) isolates the values in Theme that are City, and 2) isolates the values in These that are not Friends.
...
The next important skill is to know how to convert the text values of the levels of a qualitative variable into indicator variables. This is useful for understanding how R treats qualitative information when used in a linear model. The `Theme` variable has 3 possible values: City, Friends, DUPLO\\xae. It is sometimes helpful to create a variable that does a similar task to subsetting, but instead indicates whether a row is a certain value (indicated with a 1) or nor (indicated with a 0).

We can create these new *indicator variables* using `ifelse()` functions. These functions ask the question "Is this row equal to this value?", and then you give it an option for what to do if the answer is yes, and then what to do if the answer is no. In our case, we would say `ifelse(lego$Theme == 'DUPLO\xae', 1, 0)` which says if the value is DUPLO\\xae, assign it a value of 1, otherwise give it a value 0. Try this for the City value and the Friends value.
...
# to indicate the values equal to Friends, use
lego$Theme == "Friends"
# remember that your indicator that highlights only Friends sets is called
friends
```

Lastly, we can see how we can use this type of variable in a simple linear model. When the levels are words/text, we can simply write the variable name as a predictor and the model will understand to treat it as qualitative.
...
lm(Amazon_Price ~ as.factor(Theme), data=lego)
```
...
<div id="model1-hint">
**Hint:** Remember that the predictor we want to include is `Theme`.
</div>

What we see here is that the coefficients are labelled with the value of the qualitative predictor, and that we only see two out of the three levels present. This is because the intercept is actually equal to the average value of Amazon_Price among the City sets: E(Y | X = "City") =  `r mean(lego$Amazon_Price[lego$Theme=="City"])`. The slope for Friends is the difference between the mean Amazon Price of City sets and Friends sets: E(Y | X = "Friends") - E(Y | X = "City") =  `r mean(lego$Amazon_Price[lego$Theme=="Friends"]) - mean(lego$Amazon_Price[lego$Theme=="City"])`.

We could have also fit the exact same model using indicator variables (0's and 1's), we would just need to include more than one predictor.

```{r q2, echo=F}
question("How many indicator variables would we need to get exactly the same model we just fit?",
         ...
         answer("One for Friends, one for DUPLO/xae, and one for City", message="When an indicator is 0, we know the value must be something else. When the indicator for Friends is 0, we know the value is either City or DUPLO/xae. When we also add an indicator for DUPLO/xae, and that value is 0 too, then we know the value of Theme is NOT Friends and also NOT DUPLO/xae, so it must be City. Therefore, we don't actually need 3 indicators for 3 levels. Two gives us all the information we need."),
         allow_retry = TRUE)
```
...
Sometimes, qualitative information is recorded as numbers, usually when we are dealing with a rating system. If we were to use this kind of variable as a predictor, R would treat this as a numerical variable. Let's fit a simple linear model to Amazon_Price using rating as a predictor to see what we get.
...
Since R treats the predictor as continuous, we can interpret the coefficient as we normally would. However, we should re-evaluate whether it makes sense to treat the predictor as continuous. The rating scale only goes from 1 to 5, and it cannot take values that are not integers. Further, it is inherently a qualitative measurement (someone judged the set on goodness and gave it a rating). It also may not make sense to assume that, if we compare sets with a rating of 1 and 2, it would necessarily have the same effect on the mean Amazon Price as sets with ratings of 4 and 5 (i.e. the change may not be the same for different ratings). 

Therefore, it's important to always think about the variables to best decide how to use theme in a model. We can tell R that we want it to use rating as a qualitative variable by using the `as.factor(variable)` command, which forces R to see this variable as qualitative rather than numerical. Try this below to see how it changes the model estimates.
...
Notice that the rating of 1 is not present in the model, as the intercept captures that information. Again we would interpret the coefficients as the change in the average Amazon Price of a set with rating of X compared to a rating of 1.
...
The results of a model that uses ratings as a categorical predictor shows us that, compared to a rating of 1, sets with other ratings change the mean Amazon Price by different amounts, so it may not be reasonable to treat this predictor as continuous with a constant effect on the response.

## Section 2: Indicators as Main Effects

Let's look at how we can use categorical variables as predictors in a linear regression model and what function they perform in these models.

We will look at the relationship between the Amazon selling price (Y, Amazon_Price) and the Number of Pieces in the set (X, Pieces). But we will also see if there is a difference in this relationship based on the Theme of the set. We have already determined that the variable Theme is qualitative and categorizes the set into a theme of either City, Friends or Duplo. For this section, we will simplify this and look at only 2 themes: City and Friends.

### Fitting Separate Simple Linear Models

To begin, we will create a scatterplot with Amazon_Price on the y-axis, and Pieces on the x-axis. We will do this in a way that R will only create the axes but will not include the points just yet. We will then add the points onto the plot, highlighting the points corresponding to City sets blue, and the points corresponding to the Friends sets red. This will give us an idea of whether there is a different relationship between Amazon_Price and Pieces for each set Theme.
...
plot(x = noDuplo$Pieces, main="Price by Pieces", xlab="Pieces", ylab="Price", type="n")

points(x = city$Pieces, pch = 16, col="blue")

points(x = friends$Pieces, pch = 16, col="red")
```
...
### Adding the Set Themes as a Predictor

From the previous portion, we have noticed that when we fit separate simple linear models for each of the different sets, we find a common slope but a different intercept. To better see this with our data, we can augmnent our scatterplot to include these estimated trends.
...
We did not need to fit two separate simple linear models to be able to estimate the trend between Amazon Price and Number of Pieces for the two Themes. Instead, we could have use the categorical information in Theme as another predictor. We will do this now. Remember that we don't want to use the whole dataset because we don't want to include the DUPLO sets, so we will fit the model using the `noDuplo` dataset we created earlier.

To fit the same trend, we can simple include the Theme variable as a predictor in the model. When we fit a multiple linear model, we do so with the following syntax: `lm(y_variable ~ predictor1 + predictor2, data=dataset)`. Try this now using Amazon_Price as the response, and Pieces and Theme as the two predictors, on the dataset noDuplo.
...
You should notice that your intercept in the multiple linear model is estimated to be about 10 with an estimated slope on the indicator variable for Friends of about -7. If we think about how we interpret these values, our intercept is the mean Amazon Price for a *City* set with 0 pieces. This value is about 10, which is very close to the 9.4 we saw for an intercept in the simple linear model for City. So the intercept in the multiple linear model nearly matches the one from the City simple model.

Also, we would interpret the slope on the indicator of a Friends set to be the difference in the average Amazon Price for a Friends set with a fixed number of pieces compared to a City set. This value is -7. When we look at the difference in the intercepts between the two simple linear models, we see that this is approximately -7. So when we add the categorical information into the model rather than fit two separate models, we estimate nearly the same trend, with less work.
...
Like we did in Section 2, we will begin by plotting a scatterplot of our data. We want to plot `Amazon_Price` on the y-axis and `Pieces` on the x-axis. This time, we will highlight groups of LEGO sets based on their `Theme` (City or DUPLO). We will highlight the City sets in blue and the DUPLO sets in orange. Complete the code below to create this plot.
...
It appears that once again, we may have a different relationship between Amazon Price and Pieces depending on which set grouping we consider. To see whether this is the case (i.e. that we get a different relationship), we will fit two simple linear models again. In both cases, we want to estimate the relationship between Amazon Price and Pieces but in one we will only use the City sets and in the other we will only use the DUPLO sets.
...
### Adding the Set Themes in an Interaction

From the previous portion, we have noticed that when we fit separate simple linear models for each of the different sets, we find a common intercept but a different slope. To better see this with our data, we can augment our scatterplot to include these estimated trends.
...
To fit the same trend, we can simple include the Theme variable as an interaction with Pieces in the model. When we fit an interaction in a multiple linear model, we do so with the following syntax: `lm(y_variable ~ predictor1 + predictor2:predictor1, data=dataset)`. This makes it so that the effect of predictor1 changes depending on which group of predictor2 we are looking at.
...
### Estimating Different Slopes and Intercepts for Three Groups
...
lm(Amazon_Price ~ Theme + Pieces:Theme, data = lego)
```
...
If you look closely at the model estimates, you will notice that the slope for the interaction of Pieces and Friends Theme is very small and close to 0. We also noticed through our explorations that the Friends and City Themes had almost the same slope in their respective simple linear models. So  you might wonder whether it's necessary to estimate a value for this slope at all.
...
ind1 <- ifelse(lego$Theme == "City", 1, 0)
