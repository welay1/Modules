...
Notice that the intercept of $10.02$ is the mean Amazon Price for a "City" set with 0 pieces. Recall that the intercept from the simple linear model for City was $9.4$. Therefore, the intercept for the multiple linear model is very similar to the intercept from the corresponding simple model.  
...
If you look closely at the model estimates, you will notice that the slope for the interaction of `Pieces` and the theme Friends is very small and close to 0. We also noticed through our explorations that the themes Friends and City had almost the same slope in their respective simple linear models. So  you might wonder whether it's necessary to estimate a value for this slope at all.
...
To put it all together, explore how the simple linear model compares to the complete interaction model both graphically and in terms of the RSS below. Which model do you think more accurately describes the `lego` data? 

```{r, context="render", echo=FALSE}
withMathJax()
radioButtons("model", "Select a model:", 
             c("$$\\hat{Y} = \\hat{\\beta_0} + \\hat{\\beta_1}\\text{Pieces}$$",
               "$$\\hat{Y} = \\hat{\\beta_0} + \\hat{\\beta_1}\\text{Pieces}+ \\hat{\\beta_2}\\text{Theme} + \\hat{\\beta_3}\\text{Pieces}\\times\\text{Theme}$$")
             )
...
```{r, context="server"}
    ...
    output$plot <- renderPlot({
      ...
      if(input$model == "$$\\hat{Y} = \\hat{\\beta_0} + \\hat{\\beta_1}\\text{Pieces}$$"){
        mod <- lm(Amazon_Price ~ Pieces, data=lego)
        p <- ggplot(data = lego, aes(x = Pieces, y = Amazon_Price, color=Theme)) + geom_point() +
      ...
      mod <- lm(Amazon_Price ~ Pieces*Theme, data=lego)
              
      p <- ggplot(data = lego, aes(x = Pieces, y = Amazon_Price, color=Theme)) + geom_point() +
                geom_abline(intercept=coef(slr_city)[1], slope=coef(slr_city)[2]) + 
                geom_abline(intercept=coef(slr_duplo)[1], slope=coef(slr_duplo)[2]) +
                geom_abline(intercept=coef(slr_friends)[1], slope=coef(slr_friends)[2])
      }
