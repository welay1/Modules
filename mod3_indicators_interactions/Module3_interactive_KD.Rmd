...
* How to manipulate qualitative data

* How to include qualitative variables in regression models and how to interpret their effects on the response

* How to include interaction terms in a linear model and how to interpet their effects on the response

## Motivation

We have already seen that a simple linear model estimates the population relationship between a response (Y) and a predictor (X) using a sample. The population relationship is $$Y = \beta_0 + \beta_1 X + \epsilon$$ where we use the process of least squares to estimate $$\hat{y} = \hat{\beta_0} + \hat{\beta_1}X$$, where $\hat{\beta_0}$ is the intercept and $\hat{\beta_1}$ is the slope. 


We know that more complex relationships with multiple predictors can be modeled in the form $$Y = \beta_0 + \beta_1 X_1 + \beta_2 X_2 + \ldots + \beta_p X_p + \epsilon.$$ While it is common that predictors are *quantitative*, there can be *qualitative* properties that inform the population relationship. 
...
In the code chunk below, identify the levels of the variable `Theme`, `Pieces`, and `rating`. and the number of observations in each level. You can do this using the `table()` function, with the syntax `table(data$variable)`. 
...
*However* -- does it make sense to treat the predictor as continuous? The rating scale only goes from 1 to 5, and it can only take integer values. Further, it is inherently a qualitative measurement (the set is rated on "goodness"). It also may not make sense to assume that, if we compare sets with a rating of 1 and 2, it would necessarily have the same effect on the mean Amazon Price as sets with ratings of 4 and 5 (i.e. the price change may differ for different ratings). 
...
TODO: Insert interactive piece

Congratulations, you have completed this module! Hopefully by now, you are familiar with:

* Manipulating qualitative data

* Including qualitative variables in regression models and interpreting their effects on the response

* Including interaction terms in a linear model and interpreting their effects on the response