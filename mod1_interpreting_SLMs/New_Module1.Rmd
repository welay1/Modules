...
Suppose your friend Nancy started a cleaning company, but the company is not profitable yet. Nancy is suspicious that some employees are unproductive. When more employees are working, Nancy expects more rooms to be cleaned, but this doesn’t seem to happen. Nancy asks for your help to better understand the relationship between the number of people working and the number of rooms cleaned over the past year. 

You ask Nancy for records of the number of workers (CrewSize, the X variable) and the number of rooms cleaned (Rooms, the Y value) from the past year **to test if more workers lead to more cleaned rooms.** Unfortunately, **February**, rather than the entire year. 
...
To begin, let's produce a 5-number summary of each variable in the data. The name of the dataset is "CleaningData" and it is loaded for you to use. You can type your code into the box below. If you get stuck, click the hint button to help you with the code and syntax. 
...
ggplot(data, aes( ___ , ___ )) + 
  geom_point()+ 
  labs(x = "Number of Crew Members", y = "Number of Rooms"), 
...
TODO: insert plot image here. 

Each cluster represents the **conditional distribution** of rooms cleaned ($Y$) for a specific crew size ($X$). We can examine the average Y within each distribution.
