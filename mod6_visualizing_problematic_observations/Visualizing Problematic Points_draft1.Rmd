...
Let's now also take a look at the dimension of the dataset 'cars.' *Dimensions* refer to the number of rows and columns in the dataset. Recall that in a statistical context, rows represent individual observations or data points, while columns represent variables or attributes associated with each observation.
...
Another way to observe (visualise) the association between two variables is through a scatterplot. After fitting a simple linear regression between Speed (X) and Distance (Y), we can improve our understanding by plotting these variables and overlaying the estimated regression line on the graph.
...
question("When would you expect a leverage point to impact the estimated regression coefficients of a model?",
  answer("Always", message = "Incorrect. Not all leverage points have a significant impact on regression coefficients; it depends on whether they also are outliers affecting the model's slope."),
  answer("Sometimes", correct = TRUE, message = "Well done! Leverage points can impact the estimated regression coefficients, but this generally happens if they significantly differ in value from the rest of the data and are outliers."),
  answer("Never", message = "Incorrect. While not all leverage points affect regression coefficients, there are instances where their impact is significant, especially if they are outliers."),
  allow_retry = TRUE
)
```
...
question("Would you expect an outlier point to impact the estimated regression coefficients of a model?",
  answer("Always", message = "Incorrect. While outliers can have a significant impact, their effect on regression coefficients depends on their position relative to the rest of the data and the robustness of the regression method used."),
  answer("Sometimes", correct = TRUE, message = "Well done! Outliers can impact the estimated regression coefficients if they significantly deviate from other data points, especially in small samples or with certain types of regression analysis."),
  answer("Never", message = "Incorrect. Outliers can and do affect regression coefficients under certain conditions, such as when they are significantly distant from the bulk of the data, potentially skewing the analysis."),
  allow_retry = TRUE
)
```
...
output$interactivePlot <- renderPlot({
  #new coordinate
  library(ggplot2)
  new_point <- data.frame(speed = input$new_x, dist = input$new_y)
  new_dataset <- rbind(cars, new_point)
   
  model1 <- lm(dist ~ speed, data = cars)
  modelnew <- lm(dist ~ speed, data = new_dataset)
   
   ggplot() + geom_point(data=cars, color = "blue", aes(x = speed, y=dist), size = 1, shape = 4) + geom_point(data = new_point, color = "red", size = 2, aes(x = speed, y=dist)) + geom_smooth(data=cars, method = "lm", colour = "blue", aes(x = speed, y=dist), fill= "blue", alpha = 0.1, se = FALSE) + geom_smooth(data=new_dataset, method = "lm", colour = "red", aes(x = speed, y=dist ), fill ="red", alpha = 0.1, se = FALSE) + theme_minimal() +xlim(0,35) +ylim(0,150) + geom_text(data = data.frame(speed = 0, dist = 150), aes(label = paste("Original Model: ", paste("y =", round(coefficients(model1)[1], 2), "+", round(coefficients(model1)[2], 2) ,"x")), x = speed, y = dist), color = "blue", hjust = 0, vjust = 1) +
  geom_text(data = data.frame(speed = 0, dist = 145), aes(label = paste("New Model: ", paste("y =", round(coefficients(modelnew)[1], 2), "+", round(coefficients(modelnew)[2], 2), "x")), x = speed, y = dist), color = "red", hjust = 0, vjust = 1) 
...
fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      sliderInput("new_x", "New Point X Coordinate:", min = 0, max = 35, value = 15, step = 5),
      sliderInput("new_y", "New Point Y Coordinate:", min = 0, max = 140, value = 40, step = 5)
    ),
    mainPanel(
...
```{r interactive_q1, echo=FALSE}
question("Using what you know, and the interactive plot, which point do you think is both an outlier and a leverage point?",
         answer("(15,140)", message = "This is an outlier point as the response y deviates from the general trend of the rest of the data. Try again!"),
         answer("(35,40)", message = "This is a leverage point as it has an extreme x-value. Try again!"), 
         answer("(0,140)", correct = TRUE, message = "Well done!"), 
         answer("(15,40)", message = "This is not a problematic point. Try again!"), 
         allow_retry = TRUE)
```

----------------------------------------

```{r interactive_q2, echo=FALSE}
question("What can the point at (10,140) be categorised as?",
         answer("It is a leverage point causing minimal impact.", message = "Recall that for a point to be considered a leverage point, in the case of a single predictor, it must have an extreme x value. Try again!"),
         answer("It is a leverage point causing significant impact.", message = "Recall that for a point to be considered a leverage point, in the case of a single predictor, it must have an extreme x value. Try again!"), 
         answer("It is an outlier point causing minimal impact.", message = "While this point is an outlier as the response y does not follow the general trend of the rest of the data, it impacts the regression coefficients. Try again!"), 
         answer("It is an outlier point causing significant impact.", correct = TRUE, message = "Well done!"), 
         allow_retry = TRUE)
```

```{r interactive_q3, echo=FALSE}
question("What can the point at (0,35) be categorised as?",
         answer("It is a leverage point causing minimal impact.", message = "While this point is a leverage point as it has an extreme x-value, it impacts the regression coefficients. Try again!"),
         answer("It is a leverage point causing significant impact.", correct = TRUE, message = "Well done!"), 
         answer("It is an outlier point causing minimal impact.", message = "Recall that for a point to be considered an outlier, the response y must deviate from the general trend of the rest of the data. Try again!"), 
         answer("It is an outlier point causing significant impact.", message = "Recall that for a point to be considered an outlier, the response y must deviate from the general trend of the rest of the data. Try again!"), 
         allow_retry = TRUE)
```
